#!/usr/bin/env python3
"""交互因子 IC 计算器: interaction_ma5_dev__ret3d_abs (v2.48, 2026-06-24)
...
边界处理 (继承 calculate_interaction_ma5_dev__ret3d_abs):
...
from data_fetchers.factor_calculator import calculate_interaction_ma5_dev__ret3d_abs
...
SPEC = register_factor(
    FactorSpec(
        factor_name="interaction_ma5_dev__ret3d_abs",
        factor_col="interaction_ma5_dev__ret3d_abs",
        calculation=calculate_interaction_ma5_dev__ret3d_abs,
    )
)


def main():
    """interaction_ma5_dev__ret3d_abs 因子 IC 计算 CLI 主入口."""
    parser = argparse.ArgumentParser(description="交互因子 IC 计算器: MA5偏离__ret3d_abs")
    ...
    logger.info("interaction_ma5_dev__ret3d_abs 因子IC计算完成")
    return result
...
if __name__ == "__main__":
    try:
        ...
        logger.error("interaction_ma5_dev__ret3d_abs 因子IC计算失败: %s", e)
        sys.exit(5)
    except Exception:
