#!/usr/bin/env python3
"""交互因子 IC 计算器: interaction_intraday__ret1d_pos (v2.48, 2026-06-24)
...
边界处理 (继承 calculate_interaction_intraday__ret1d_pos):
...
from data_fetchers.factor_calculator import calculate_interaction_intraday__ret1d_pos
...
SPEC = register_factor(
    FactorSpec(
        factor_name="interaction_intraday__ret1d_pos",
        factor_col="interaction_intraday__ret1d_pos",
        calculation=calculate_interaction_intraday__ret1d_pos,
    )
)


def main():
    """interaction_intraday__ret1d_pos 因子 IC 计算 CLI 主入口."""
    parser = argparse.ArgumentParser(description="交互因子 IC 计算器: 日内强度__ret1d_pos")
    ...
    logger.info("interaction_intraday__ret1d_pos 因子IC计算完成")
    return result
...
if __name__ == "__main__":
    try:
        ...
        logger.error("interaction_intraday__ret1d_pos 因子IC计算失败: %s", e)
        sys.exit(5)
    except Exception:
