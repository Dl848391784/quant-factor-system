...
> 版本: v2.0
> 生成时间: 2026-05-23 22:55 北京时间
> 实测数据时间: 2026-05-23 22:55 北京时间（运行验证通过）
> 脚本: ic_volume_ratio_1d.py（252行）
> 更新内容:
>   1. v1.0 首次创建流程文档
...
>      - 运行验证通过（SKIP模式正常工作）
>  10. v2.0 CLI参数解析+步骤日志完善（2026-05-23 22:55）：
>      - 添加 CLI 参数解析：--force-full、--output、--min-stocks（与 ic_rsi_1d.py 保持一致）
>      - FULL 模式添加 [2/4] 步骤日志（因子已在缓存中，跳过因子计算）
>      - 移除冗余 convert_to_native_types 调用（save_ic_result 内部已处理）
>      - 代码行数：236行 → 252行
>      - CLI 参数验证通过：--force-full 成功触发全量计算
