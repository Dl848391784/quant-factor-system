...
- 物理含义: 近5日均量与近10日均量之比（量能衰减趋势）
- 企稳信号: 值 < 1 = 近期量能小于远期（量能衰减，卖盘衰竭，企稳特征）
- 遵循 H5: IC方向不预判，由数据决定
...
from data_fetchers.factor_calculator import calculate_volume_decay_rate
...
try:
    SPEC = register_factor(
        FactorSpec(
            factor_name="volume_decay_rate",
            factor_col="volume_decay_rate",
            calculation=calculate_volume_decay_rate,
        )
    )
except SpecRegistrationError as e:
    logger.critical(
        "FactorSpec 注册失败 (factor=volume_decay_rate): %s (%s) (truncated to <=%d chars)",
        str(e)[:_MAX_ERR_LEN],
        type(e).__name__,
        _MAX_ERR_LEN,
    )
...
def _build_parser():
    ...
    parser = argparse.ArgumentParser(description="量能衰减因子 IC 计算器")
...
def main(args=None):
    ...
    try:
        log_factor_summary(result, "量能衰减因子", logger)
    except Exception as e:
        raise SummaryLogError(
            "log_factor_summary 摴要输出阶段失败（因子计算 result 已成功生成；"
            "故障源 = 摴要日志层而非 run_factor_ic 业务路径）"
        ) from e
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.error(
            "摘要输出阶段失败（因子计算 result 已成功生成；故障源 = 摴要日志层；原因: %s）",
            e.__cause__ or e,
        )
        sys.exit(3)
    except FactorCalcError as e:
        logger.error(
            "量能衰减因子 IC计算失败 (factor=%s, min_stocks=%s, force_full=%s): %s",
            SPEC.factor_name,
