...
from data_fetchers.factor_calculator.volume_price import calculate_volume_decay_rate
...
def main() -> None:
    ...
    try:
        ...
        try:
            ...
            logger.error(
                "日志摘要记录失败 (factor=volume_decay_rate): %s (%s) (truncated to <=%d chars)",
                str(e)[:_MAX_ERR_LEN],
                type(e).__name__,
                _MAX_ERR_LEN,
            )
        ...
        logger.critical(
            "数据 schema 不匹配 (factor=volume_decay_rate): %s (%s)",
            str(e)[:_MAX_ERR_LEN],
            type(e).__name__,
        )
        exit_code = 4
        ...
        logger.exception(
            "因子计算失败 (factor=volume_decay_rate): %s (%s)",
            str(e)[:_MAX_ERR_LEN],
            type(e).__name__,
        )
        exit_code = 5
