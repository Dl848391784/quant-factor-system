...
- 物理含义: 当日换手率与5日平均换手率之比（换手率衰减）
- 企稳信号: 值 < 1 = 当日换手低于5日均（交易参与度下降，卖盘衰竭）
- 遵循 H5: IC方向不预判，由数据决定
...
def main(args=None):
    ...
    try:
        log_factor_summary(result, "换手率衰减因子", logger)
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.error(
            "换手率衰减因子 IC计算失败 (factor=%s, min_stocks=%s, force_full=%s): %s",
            SPEC.factor_name,
