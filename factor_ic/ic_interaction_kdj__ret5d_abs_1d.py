#!/usr/bin/env python3
"""交互因子 IC 计算器: interaction_kdj__ret5d_abs (v2.48, 2026-06-24)
...
边界处理 (继承 calculate_interaction_kdj__ret5d_abs):
...
from data_fetchers.factor_calculator import calculate_interaction_kdj__ret5d_abs
...
SPEC = register_factor(
    FactorSpec(
        factor_name="interaction_kdj__ret5d_abs",
        factor_col="interaction_kdj__ret5d_abs",
        calculation=calculate_interaction_kdj__ret5d_abs,
    )
)


def main():
    """interaction_kdj__ret5d_abs 因子 IC 计算 CLI 主入口."""
    parser = argparse.ArgumentParser(description="交互因子 IC 计算器: KDJ_J__ret5d_abs")
    ...
    logger.info("interaction_kdj__ret5d_abs 因子IC计算完成")
    return result
...
if __name__ == "__main__":
    try:
        ...
        logger.error("interaction_kdj__ret5d_abs 因子IC计算失败: %s", e)
        sys.exit(5)
    except Exception:
