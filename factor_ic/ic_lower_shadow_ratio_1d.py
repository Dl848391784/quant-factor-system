...
from data_fetchers.factor_calculator import calculate_lower_shadow_ratio
...
try:
    SPEC = register_factor(
        FactorSpec(
            ...
            factor_col="lower_shadow_ratio",
            calculation=calculate_lower_shadow_ratio,
        )
    )
except SpecRegistrationError as e:
    logger.critical(
        "FactorSpec 注册失败 (factor=lower_shadow_ratio): %s (%s) (truncated to <=%d chars)",
        str(e)[:_MAX_ERR_LEN],
        type(e).__name__,
        _MAX_ERR_LEN,
    )
...
def _build_parser():
    ...
    parser = argparse.ArgumentParser(description="下影线比因子 IC 计算器")
...
def main(args=None):
    ...
    try:
        log_factor_summary(result, "下影线比因子", logger)
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.error(
            "下影线比因子 IC计算失败 (factor=%s, min_stocks=%s, force_full=%s): %s",
            SPEC.factor_name,
