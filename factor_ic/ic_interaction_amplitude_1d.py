#!/usr/bin/env python3
"""交互因子 IC 计算器：interaction_amplitude（v2.36, 2026-06-22）
...
- amplitude_z = 截面 z-score 化的 amplitude（振幅 z-score）
- 含义: 弱势 × 高振幅 = 反弹型信号；强势 × 高振幅 = 高位风险

第一性原理（详见 skill factor-development ref conditional-ic-analysis.md）:
  IC = corr(因子, 收益) 是无条件相关系数，假设"因子-收益关系在所有条件下相同"。
  实证 amplitude 在弱势子样本(RSI<30)中 IC=+0.083，全样本 IC=-0.022（被强势股稀释）。
  交互因子用乘法把"条件方向"提取到无条件 IC 上 → 期望全样本 IC≈+0.020 翻正。

边界处理（继承 calculate_interaction_amplitude）:
...
创建日期: 2026-06-22
版本历史:
  v1.0 (2026-06-22): 初始版本，复用 factor_calculator.calculate_interaction_amplitude
"""
...
SPEC = register_factor(
    FactorSpec(
        factor_name="interaction_amplitude",
        factor_col="interaction_amplitude",
        calculation=calculate_interaction_amplitude,
    )
)


def main():
    """interaction_amplitude 因子 IC 计算 CLI 主入口。
    ...
    Exception
        其他未预期异常会原样向上传播。
    """
    parser = argparse.ArgumentParser(description="交互因子(振幅) IC 计算器")
    ...
    logger.info("interaction_amplitude 因子IC计算完成")
    return result
...
if __name__ == "__main__":
    try:
        ...
        logger.error("interaction_amplitude 因子IC计算失败: %s", e)
        sys.exit(5)
    except Exception:
