...
from data_fetchers.factor_calculator import calculate_industry_earnings_growth  # noqa: E402
...
def main():
    ...
    logger.info(f"启动行业盈利增长因子IC计算: min_stocks={args.min_stocks}, force_full={args.force_full}")

    result = run_complex_factor_ic(
        factor_name="industry_earnings_growth",
        factor_col="industry_earnings_growth",
        factor_cols=["date", "asset"],
        custom_factor_calculation=calculate_industry_earnings_growth,
...
if __name__ == "__main__":
    try:
        ...
        logger.error(f"行业盈利增长因子IC计算失败: {e}")
        sys.exit(1)
    except Exception:
