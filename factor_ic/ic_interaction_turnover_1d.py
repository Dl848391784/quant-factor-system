#!/usr/bin/env python3
"""交互因子 IC 计算器：interaction_turnover（v2.36, 2026-06-22）
...
- turnover_z = 截面 z-score 化的 turnover_rate（换手率 z-score）
...
第一性原理（详见 skill factor-development ref conditional-ic-analysis.md）:
  ...
  实证 turnover_rate 在弱势子样本(RSI<30)中 IC=+0.044，全样本 IC=-0.028（被强势股稀释）。
  交互因子用乘法把"条件方向"提取到无条件 IC 上 → 期望全样本 IC≈+0.016 翻正。

边界处理（继承 calculate_interaction_turnover）:
...
创建日期: 2026-06-22
版本历史:
  v1.0 (2026-06-22): 初始版本，复用 factor_calculator.calculate_interaction_turnover
"""
...
SPEC = register_factor(
    FactorSpec(
        factor_name="interaction_turnover",
        factor_col="interaction_turnover",
        calculation=calculate_interaction_turnover,
    )
)


def main():
    """interaction_turnover 因子 IC 计算 CLI 主入口。
    ...
    Exception
        其他未预期异常会原样向上传播。
    """
    parser = argparse.ArgumentParser(description="交互因子(换手) IC 计算器")
    ...
    logger.info("interaction_turnover 因子IC计算完成")
    return result
...
if __name__ == "__main__":
    try:
        ...
        logger.error("interaction_turnover 因子IC计算失败: %s", e)
        sys.exit(5)
    except Exception:
