...
from data_fetchers.factor_calculator import calculate_rsi_slope_3d
...
try:
    SPEC = register_factor(
        FactorSpec(
            factor_name="rsi_slope_3d",
            factor_col="rsi_slope_3d",
            calculation=calculate_rsi_slope_3d,
        )
    )
except SpecRegistrationError as e:
    logger.critical(
        "FactorSpec 注册失败 (factor=rsi_slope_3d): %s (%s) (truncated to <=%d chars)",
        str(e)[:_MAX_ERR_LEN],
        type(e).__name__,
        _MAX_ERR_LEN,
    )
...
def _build_parser():
    ...
    parser = argparse.ArgumentParser(description="RSI 3日斜率因子 IC 计算器")
...
def main(args=None):
    ...
    try:
        log_factor_summary(result, "RSI 3日斜率因子", logger)
    ...
    logger.info(
        "RSI 3日斜率因子 IC计算完成: IC均值=%s, ICIR=%s",
        ic_mean_str,
        icir_str,
    )
    return result
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.error(
            "RSI 3日斜率因子 IC计算失败 (factor=%s, min_stocks=%s, force_full=%s): %s",
            SPEC.factor_name,
