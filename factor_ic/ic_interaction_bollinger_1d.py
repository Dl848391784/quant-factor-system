#!/usr/bin/env python3
"""交互因子 IC 计算器：interaction_bollinger（v2.37, 2026-06-22）
...
SPEC = register_factor(
    FactorSpec(
        factor_name="interaction_bollinger",
        factor_col="interaction_bollinger",
        calculation=calculate_interaction_bollinger,
    )
)


def main():
    parser = argparse.ArgumentParser(description="interaction_bollinger 交互因子 IC 计算器")
...
if __name__ == "__main__":
    try:
        ...
        logger.error("interaction_bollinger 因子IC计算失败: %s", e)
        sys.exit(5)
    except Exception:
