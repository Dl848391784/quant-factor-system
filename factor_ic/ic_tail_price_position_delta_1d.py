...
def main():
    ...
    parser = argparse.ArgumentParser(description="尾盘位置差分因子 IC 计算器")
    ...
    # 调用前日志
    logger.info(f"启动尾盘位置差分因子IC计算: min_stocks={args.min_stocks}, force_full={args.force_full}")

    # 使用公共模块主入口（遵循 PROJECT.md 强制复用规范）
    result = run_complex_factor_ic(
        factor_name="tail_price_position_delta",
        factor_col="tail_price_position_delta",
        factor_cols=["date", "asset", "tail_price_position"],  # 需要原始因子列进行差分计算
        custom_factor_calculation=calculate_tail_price_position_delta,
...
if __name__ == "__main__":
    try:
        ...
        logger.error(f"尾盘位置差分因子IC计算失败: {e}")
        sys.exit(1)
    except Exception:
