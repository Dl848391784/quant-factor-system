...
因子定义：
- volume_shrink_rate = volume / rolling(volume, 5).mean()
- 含义: 缩量=卖盘衰竭，<1为缩量信号
- 遵循 H5: IC方向不预判，由数据决定
...
from data_fetchers.factor_calculator import calculate_volume_shrink_rate
...
try:
    SPEC = register_factor(
        FactorSpec(
            factor_name="volume_shrink_rate",
            factor_col="volume_shrink_rate",
            calculation=calculate_volume_shrink_rate,
        )
    )
except SpecRegistrationError as e:
    logger.critical(
        "FactorSpec 注册失败 (factor=volume_shrink_rate): %s (%s) (truncated to <=%d chars)",
        str(e)[:_MAX_ERR_LEN],
        type(e).__name__,
        _MAX_ERR_LEN,
    )
...
def _build_parser():
    ...
    parser = argparse.ArgumentParser(description="缩量率因子 IC 计算器")
...
def main(args=None):
    ...
    try:
        log_factor_summary(result, "缩量率因子", logger)
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.error(
            "缩量率因子 IC计算失败 (factor=%s, min_stocks=%s, force_full=%s): %s",
            SPEC.factor_name,
