...
def main():
    ...
    logger.info(f"启动5日均线偏离度因子IC计算: min_stocks={args.min_stocks}, force_full={args.force_full}")

    result = run_complex_factor_ic(
        factor_name="ma5_deviation",
        factor_col="ma5_deviation",
        factor_cols=["date", "asset", "close"],
        custom_factor_calculation=calculate_ma5_deviation,
...
if __name__ == "__main__":
    try:
        ...
        logger.error(f"5日均线偏离度因子IC计算失败: {e}")
        sys.exit(1)
    except Exception:
