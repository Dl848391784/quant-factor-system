#!/usr/bin/env python3
"""交互因子 IC 计算器: interaction_amp_compression__ret3d_neg (v2.48, 2026-06-24)
...
边界处理 (继承 calculate_interaction_amp_compression__ret3d_neg):
...
SPEC = register_factor(
    FactorSpec(
        factor_name="interaction_amp_compression__ret3d_neg",
        factor_col="interaction_amp_compression__ret3d_neg",
        calculation=calculate_interaction_amp_compression__ret3d_neg,
    )
)


def main():
    """interaction_amp_compression__ret3d_neg 因子 IC 计算 CLI 主入口."""
    parser = argparse.ArgumentParser(description="交互因子 IC 计算器: 振幅压缩__ret3d_neg")
    ...
    logger.info("interaction_amp_compression__ret3d_neg 因子IC计算完成")
    return result
...
if __name__ == "__main__":
    try:
        ...
        logger.error("interaction_amp_compression__ret3d_neg 因子IC计算失败: %s", e)
        sys.exit(5)
    except Exception:
