#!/usr/bin/env python3
"""test_ic_tail_price_position_delta_1d 测试用例

测试脚本: factor_ic/ic_tail_price_position_delta_1d.py
因子计算: data_fetchers/factor_calculator.py::calculate_tail_price_position_delta
规范文档: factor_ic/MODULE.md

运行: pytest factor_ic/test_cases/test_ic_tail_price_position_delta_1d.py -v
"""
...
from data_fetchers.factor_calculator import calculate_tail_price_position_delta
...
class TestOutputPath:
    ...
    def test_output_path_format(self):
        ...
        path = get_ic_output_path("tail_price_position_delta")
        assert path.name == "ic_tail_price_position_delta_analysis_result.json"
...
class TestCalculateTailPricePositionDelta:
    """因子计算验证（导入即可，详细计算逻辑在 data_fetchers 测试覆盖）"""

    def test_calculator_importable(self):
        """计算函数可导入且可调用"""
        assert callable(calculate_tail_price_position_delta)
...
class TestOutputStructure:
    ...
    def test_output_file_exists_after_run(self):
        ...
        path = get_ic_output_path("tail_price_position_delta")
        if not path.exists():
            pytest.skip("输出文件不存在，需要先运行 ic_tail_price_position_delta_1d.py")
