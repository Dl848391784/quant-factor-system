...
class TestCalculateAmplitude:
    ...
    def test_basic_calculation(self):
        """基本计算测试"""
        df = pd.DataFrame({"close": [10.0, 12.0, 11.0], "high": [12.0, 13.0, 12.0], "low": [9.0, 11.0, 10.0]})
    ...
    def test_upper_bound(self):
        """上限测试：振幅较大"""
        df = pd.DataFrame({"close": [10.0], "high": [15.0], "low": [5.0]})
        result = calculate_amplitude(df)
    ...
    def test_zero_amplitude(self):
        """振幅为零测试：一字涨跌停"""
        df = pd.DataFrame({"close": [10.0], "high": [10.0], "low": [10.0]})
        result = calculate_amplitude(df)
    ...
    def test_zero_close_handling(self):
        """收盘价为零测试：无效数据"""
        df = pd.DataFrame({"close": [0.0], "high": [12.0], "low": [9.0]})
        result = calculate_amplitude(df)
    ...
    def test_multiple_rows(self):
        """多行数据处理测试"""
        df = pd.DataFrame(
            {"close": [10.0, 12.0, 11.0, 0.0], "high": [12.0, 13.0, 12.0, 11.0], "low": [9.0, 11.0, 10.0, 9.0]}
        )
        result = calculate_amplitude(df)
    ...
    def test_nan_handling(self):
        """NaN 值处理测试"""
        df = pd.DataFrame({"close": [10.0, np.nan, 12.0], "high": [12.0, 13.0, 12.0], "low": [9.0, 11.0, 10.0]})
        result = calculate_amplitude(df)
