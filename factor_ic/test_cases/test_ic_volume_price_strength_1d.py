#!/usr/bin/env python3
"""test_ic_volume_price_strength_1d 测试用例

测试脚本: factor_ic/ic_volume_price_strength_1d.py
因子计算: data_fetchers/factor_calculator.py::calculate_volume_price_strength
规范文档: factor_ic/MODULE.md

运行: pytest factor_ic/test_cases/test_ic_volume_price_strength_1d.py -v
"""
...
class TestOutputPath:
    ...
    def test_output_path_format(self):
        ...
        path = get_ic_output_path("volume_price_strength")
        assert path.name == "ic_volume_price_strength_analysis_result.json"
...
class TestCalculateVolumePriceStrength:
    ...
    def test_calculator_importable(self):
        """计算函数可导入且可调用"""
        assert callable(calculate_volume_price_strength)
...
class TestOutputStructure:
    ...
    def test_output_file_exists_after_run(self):
        ...
        path = get_ic_output_path("volume_price_strength")
        if not path.exists():
            pytest.skip("输出文件不存在，需要先运行 ic_volume_price_strength_1d.py")
