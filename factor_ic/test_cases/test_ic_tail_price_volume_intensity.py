...
class TestCalculateTailPriceVolumeIntensity:
    ...
    def test_normal_calculation(self):
        ...
        try:
            ...
            assert (
                abs(
                    result_df.loc[result_df["asset"] == "000001", "tail_price_volume_intensity"].values[0]
                    - expected_000001
                )
                < 0.001
            )
    ...
    def test_zero_price_protection(self):
        """除零防护：prices[0] 接近零"""
        factor_df = pd.DataFrame({"date": ["2026-06-01"], "asset": ["000001"], "volume": [1000000]})
    ...
    def test_zero_volume_protection(self):
        ...
        mock_tail_df = pd.DataFrame(
            {"date": ["2026-06-01"], "asset": ["000001"], "prices": [[10.0] * 13], "volumes": [[10000] * 13]}
        )
