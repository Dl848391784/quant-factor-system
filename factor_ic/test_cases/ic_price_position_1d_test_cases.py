...
class TestCalculatePricePosition:
    ...
    def test_upper_bound(self):
        """上限测试：收盘等于最高"""
        df = pd.DataFrame({"close": [12.0], "high": [12.0], "low": [10.0]})
        result = calculate_price_position(df)
    ...
    def test_lower_bound(self):
        """下限测试：收盘等于最低"""
        df = pd.DataFrame({"close": [10.0], "high": [12.0], "low": [10.0]})
        result = calculate_price_position(df)
    ...
    def test_midpoint(self):
        """中位测试：收盘在振幅中间"""
        df = pd.DataFrame({"close": [11.0], "high": [12.0], "low": [10.0]})
        result = calculate_price_position(df)
    ...
    def test_multiple_rows(self):
        """多行数据处理测试"""
        df = pd.DataFrame(
            {"close": [10.0, 11.0, 12.0, 10.0], "high": [12.0, 12.0, 12.0, 10.0], "low": [9.0, 10.0, 11.0, 10.0]}
        )
        result = calculate_price_position(df)
    ...
    def test_nan_handling(self):
        """NaN 值处理测试"""
        df = pd.DataFrame({"close": [10.0, np.nan, 12.0], "high": [12.0, 13.0, 12.0], "low": [9.0, 11.0, 11.0]})
        result = calculate_price_position(df)
