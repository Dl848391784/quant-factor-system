#!/usr/bin/env python3
"""test_ic_tail_volume_shrink_delta_1d 测试用例

测试脚本: factor_ic/ic_tail_volume_shrink_delta_1d.py
因子计算: data_fetchers/factor_calculator.py::calculate_tail_volume_shrink_delta
规范文档: factor_ic/MODULE.md

运行: pytest factor_ic/test_cases/test_ic_tail_volume_shrink_delta_1d.py -v
"""
...
class TestOutputPath:
    ...
    def test_output_path_format(self):
        ...
        path = get_ic_output_path("tail_volume_shrink_delta")
        assert path.name == "ic_tail_volume_shrink_delta_analysis_result.json"
...
class TestOutputStructure:
    ...
    def test_output_file_exists_after_run(self):
        ...
        path = get_ic_output_path("tail_volume_shrink_delta")
        if not path.exists():
            pytest.skip("输出文件不存在，需要先运行 ic_tail_volume_shrink_delta_1d.py")
