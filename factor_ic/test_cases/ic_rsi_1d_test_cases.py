...
class TestOutputStructure:
    ...
    # 五维度字段（PROJECT.md 规范）
    STATISTICAL_SIGNIFICANCE_FIELDS = [
        "p_value",
        "p_value_display",
        "t_stat",
        "nw_lag",
    ...
    IC_DISTRIBUTION_CONSISTENCY_FIELDS = [
        "positive_ratio",
        "ic_mean_sign",
        "is_consistent",
        "consistency_type",
    ...
    def test_output_structure_has_required_fields(self, tmp_path):
        ...
        expected_structure = {
            ...
            "sample_stats": {"total_days": 520, "valid_days": 515, "avg_stocks_per_day": 4500},
            # 五维度判断（独立输出）
    ...
    def test_summary_format_with_positive_ratio(self, tmp_path):
        ...
        expected_structure = {
            ...
            "icir_stability": {"icir": 0.2889, "level": "none", "is_stable": False, "conclusion": "IC稳定性不足"},
            "ic_distribution_consistency": {
                "positive_ratio": 0.3813,
                "ic_mean_sign": "negative",
                "is_consistent": True,
...
class TestAlgorithmEquivalence:
    ...
    def test_full_incremental_same_core_function(self):
        ...
        from factor_ic.common.ic_calculator import calculate_ic_with_direction_verification, calculate_single_day_ic
        ...
        incremental_ic = calculate_single_day_ic(
            test_data, factor_col="rsi_6", return_col="forward_return", min_stocks=10
        )
        ...
        full_result = calculate_ic_with_direction_verification(
            factor_df, return_df, factor_col="rsi_6", return_col="forward_return"
        )
        full_ic = full_result["ic_series"].iloc[0]

        # 验证等价性：两者应产生相同的 IC 值
        assert incremental_ic is not None
        assert abs(incremental_ic - full_ic) < 1e-6, f"全量/增量 IC 值不一致: 全量={full_ic}, 增量={incremental_ic}"
    ...
    def test_boundary_handling_equivalence_insufficient_stocks(self):
        ...
        # 增量计算：直接调用 calculate_single_day_ic
        incremental_ic = calculate_single_day_ic(
            insufficient_data, factor_col="rsi_6", return_col="forward_return", min_stocks=10
        )
    ...
    def test_boundary_handling_equivalence_constant_factor(self):
        ...
        # 增量计算：直接调用 calculate_single_day_ic
        incremental_ic = calculate_single_day_ic(
            constant_factor_data, factor_col="rsi_6", return_col="forward_return", min_stocks=10
        )
    ...
    def test_full_incremental_equivalence_multi_day(self):
        ...
        for date, daily_data in test_df.groupby("date"):
            ic_value = calculate_single_day_ic(
                daily_data, factor_col="rsi_6", return_col="forward_return", min_stocks=10
            )
