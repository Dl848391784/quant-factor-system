...
from data_fetchers.factor_calculator import calculate_ma5_slope
...
try:
    SPEC = register_factor(
        FactorSpec(
            factor_name="ma5_slope",
            factor_col="ma5_slope",
            calculation=calculate_ma5_slope,
        )
    )
except SpecRegistrationError as e:
    logger.critical(
        "FactorSpec 注册失败 (factor=ma5_slope): %s (%s) (truncated to <=%d chars)",
        str(e)[:_MAX_ERR_LEN],
        type(e).__name__,
        _MAX_ERR_LEN,
    )
...
def _build_parser():
    ...
    parser = argparse.ArgumentParser(description="MA5 3日斜率因子 IC 计算器")
...
def main(args=None):
    ...
    try:
        log_factor_summary(result, "MA5 3日斜率因子", logger)
    ...
    logger.info(
        "MA5 3日斜率因子 IC计算完成: IC均值=%s, ICIR=%s",
        ic_mean_str,
        icir_str,
    )
    return result
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.error(
            "MA5 3日斜率因子 IC计算失败 (factor=%s, min_stocks=%s, force_full=%s): %s",
            SPEC.factor_name,
