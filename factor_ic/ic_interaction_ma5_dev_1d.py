#!/usr/bin/env python3
"""交互因子 IC 计算器：interaction_ma5_dev（v2.37, 2026-06-22）

设计依据: designs/feat_interaction_factors_batch2.md
因子定义: -z_cs(ret3d) × z_cs(ma5_deviation)
实证全样本 IC≈+0.0322
...
from data_fetchers.factor_calculator import calculate_interaction_ma5_dev
...
SPEC = register_factor(
    FactorSpec(
        factor_name="interaction_ma5_dev",
        factor_col="interaction_ma5_dev",
        calculation=calculate_interaction_ma5_dev,
    )
)


def main():
    parser = argparse.ArgumentParser(description="interaction_ma5_dev 交互因子 IC 计算器")
...
if __name__ == "__main__":
    try:
        ...
        logger.error("interaction_ma5_dev 因子IC计算失败: %s", e)
        sys.exit(5)
    except Exception:
