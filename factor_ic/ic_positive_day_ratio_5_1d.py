...
因子定义：positive_day_ratio_5 = count(close>prev_close, 5日)/5
含义：近期持续上涨=上升趋势，方向性因子。遵循 H5: IC方向不预判。
边界：前4天无完整窗口→NaN（自然排除）。

作者: 云瑶
创建日期: 2026-06-11
版本历史:
  v1.0 (2026-06-11): 初始版本，复用 factor_calculator.calculate_positive_day_ratio_5
"""
...
def main():
    """CLI 主入口"""
    parser = argparse.ArgumentParser(description="近5日阳线比例因子 IC 计算器")
    parser.add_argument("--force-full", action="store_true", help="强制全量计算")
    parser.add_argument("--min-stocks", type=int, default=DEFAULT_MIN_STOCKS, help="最小股票数")
    args = parser.parse_args()

    logger.info(f"启动近5日阳线比例因子IC计算: min_stocks={args.min_stocks}, force_full={args.force_full}")
...
if __name__ == "__main__":
    try:
        ...
        logger.error(f"近5日阳线比例因子IC计算失败: {e}")
        sys.exit(1)
    except Exception:
