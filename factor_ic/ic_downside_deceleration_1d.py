...
- 因子计算逻辑复用 data_fetchers.factor_calculator 的 calculate_downside_deceleration

因子定义：
- downside_deceleration: max(0, return_5d(t) - return_5d(t-5)) 仅当前期下跌
- 含义: 下跌股票的跌幅收窄幅度，正值=之前在跌现在跌幅收窄
...
def main(args=None):  # noqa: C901
    ...
    log_factor_summary(result, factor_name="downside_deceleration")
    sys.exit(0)
