#!/usr/bin/env python3
"""交互因子 IC 计算器: interaction_turnover__ret3d_neg (v2.48, 2026-06-24)

设计依据: designs/feat_factor_definition_destigmatization_v1.md v1.2
因子定义 (ReLU 切半轴, 纯数学命名, 无经济叙事):
- interaction_turnover__ret3d_pos = max(z_cs(return_3d), 0) × z_cs(turnover_rate)
- interaction_turnover__ret3d_neg = min(z_cs(return_3d), 0) × z_cs(turnover_rate)
- interaction_turnover__ret3d_abs =     |z_cs(return_3d)|   × z_cs(turnover_rate)

方向 (factor_direction) 由 IC 闸口数据驱动决定, 不预设叙事标签
(遵循 AGENTS.md "数据驱动原则" + backtest/MODULE.md v2.5 M17).

边界处理 (继承 calculate_interaction_turnover__ret3d_neg):
- return_3d / turnover_rate 缺失 → 交互值 NaN
...
from data_fetchers.factor_calculator import calculate_interaction_turnover__ret3d_neg
...
SPEC = register_factor(
    FactorSpec(
        factor_name="interaction_turnover__ret3d_neg",
        factor_col="interaction_turnover__ret3d_neg",
        calculation=calculate_interaction_turnover__ret3d_neg,
    )
)


def main():
    """interaction_turnover__ret3d_neg 因子 IC 计算 CLI 主入口."""
    parser = argparse.ArgumentParser(description="交互因子 IC 计算器: 换手率__ret3d_neg")
    ...
    logger.info("interaction_turnover__ret3d_neg 因子IC计算完成")
    return result
...
if __name__ == "__main__":
    try:
        ...
        logger.error("interaction_turnover__ret3d_neg 因子IC计算失败: %s", e)
        sys.exit(5)
    except Exception:
