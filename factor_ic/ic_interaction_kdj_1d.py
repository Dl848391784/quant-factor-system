#!/usr/bin/env python3
"""交互因子 IC 计算器：interaction_kdj（v2.37, 2026-06-22）

设计依据: designs/feat_interaction_factors_batch2.md
因子定义: -z_cs(ret5d) × z_cs(kdj_j)
实证全样本 IC≈+0.0286
...
from data_fetchers.factor_calculator import calculate_interaction_kdj
...
SPEC = register_factor(
    FactorSpec(
        factor_name="interaction_kdj",
        factor_col="interaction_kdj",
        calculation=calculate_interaction_kdj,
    )
)


def main():
    parser = argparse.ArgumentParser(description="interaction_kdj 交互因子 IC 计算器")
...
if __name__ == "__main__":
    try:
        ...
        logger.error("interaction_kdj 因子IC计算失败: %s", e)
        sys.exit(5)
    except Exception:
