#!/usr/bin/env python3
"""交互因子 IC 计算器: interaction_price_pos__ret1d_abs (v2.48, 2026-06-24)

设计依据: designs/feat_factor_definition_destigmatization_v1.md v1.2
因子定义 (ReLU 切半轴, 纯数学命名, 无经济叙事):
- interaction_price_pos__ret1d_pos = max(z_cs(return_1d), 0) × z_cs(price_position)
- interaction_price_pos__ret1d_neg = min(z_cs(return_1d), 0) × z_cs(price_position)
...
边界处理 (继承 calculate_interaction_price_pos__ret1d_abs):
...
from data_fetchers.factor_calculator import calculate_interaction_price_pos__ret1d_abs
...
SPEC = register_factor(
    FactorSpec(
        factor_name="interaction_price_pos__ret1d_abs",
        factor_col="interaction_price_pos__ret1d_abs",
        calculation=calculate_interaction_price_pos__ret1d_abs,
    )
)


def main():
    """interaction_price_pos__ret1d_abs 因子 IC 计算 CLI 主入口."""
    parser = argparse.ArgumentParser(description="交互因子 IC 计算器: 价格位置__ret1d_abs")
    ...
    logger.info("interaction_price_pos__ret1d_abs 因子IC计算完成")
    return result
...
if __name__ == "__main__":
    try:
        ...
        logger.error("interaction_price_pos__ret1d_abs 因子IC计算失败: %s", e)
        sys.exit(5)
    except Exception:
