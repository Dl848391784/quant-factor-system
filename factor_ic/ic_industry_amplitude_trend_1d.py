...
def main():
    ...
    logger.info(f"启动行业振幅趋势因子IC计算: min_stocks={args.min_stocks}, force_full={args.force_full}")

    result = run_complex_factor_ic(
        factor_name="industry_amplitude_trend",
        factor_col="industry_amplitude_trend",
        factor_cols=["date", "asset", "amplitude"],
        custom_factor_calculation=calculate_industry_amplitude_trend,
...
if __name__ == "__main__":
    try:
        ...
        logger.error(f"行业振幅趋势因子IC计算失败: {e}")
        sys.exit(1)
    except Exception:
