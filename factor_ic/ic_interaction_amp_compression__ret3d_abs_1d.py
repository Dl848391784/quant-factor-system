#!/usr/bin/env python3
"""交互因子 IC 计算器: interaction_amp_compression__ret3d_abs (v2.48, 2026-06-24)

设计依据: designs/feat_factor_definition_destigmatization_v1.md v1.2
因子定义 (ReLU 切半轴, 纯数学命名, 无经济叙事):
- interaction_amp_compression__ret3d_pos = max(z_cs(return_3d), 0) × z_cs(amplitude_compression)
- interaction_amp_compression__ret3d_neg = min(z_cs(return_3d), 0) × z_cs(amplitude_compression)
- interaction_amp_compression__ret3d_abs =     |z_cs(return_3d)|   × z_cs(amplitude_compression)

方向 (factor_direction) 由 IC 闸口数据驱动决定, 不预设叙事标签
(遵循 AGENTS.md "数据驱动原则" + backtest/MODULE.md v2.5 M17).

边界处理 (继承 calculate_interaction_amp_compression__ret3d_abs):
...
from data_fetchers.factor_calculator import calculate_interaction_amp_compression__ret3d_abs
...
SPEC = register_factor(
    FactorSpec(
        factor_name="interaction_amp_compression__ret3d_abs",
        factor_col="interaction_amp_compression__ret3d_abs",
        calculation=calculate_interaction_amp_compression__ret3d_abs,
    )
)


def main():
    """interaction_amp_compression__ret3d_abs 因子 IC 计算 CLI 主入口."""
    parser = argparse.ArgumentParser(description="交互因子 IC 计算器: 振幅压缩__ret3d_abs")
    ...
    logger.info("interaction_amp_compression__ret3d_abs 因子IC计算完成")
    return result
...
if __name__ == "__main__":
    try:
        ...
        logger.error("interaction_amp_compression__ret3d_abs 因子IC计算失败: %s", e)
        sys.exit(5)
    except Exception:
