...
# 尾盘数据路径
TAIL_TRADING_DATA_PATH = Path(__file__).parent.parent / 'data_fetchers' / 'result' / 'tail_trading_data.json.gz'
...
def calculate_tail_price_volume_intensity(factor_df):
    ...
    logger.info("尾盘数据合并完成: %d / %d 条匹配", 
                merged_df['prices'].notna().sum(), len(factor_df))
    ...
    logger.info("尾盘量价强度因子计算完成: %d / %d 有效 (%.1f%%)",
                valid_count, total_count, 100 * valid_count / total_count if total_count > 0 else 0)
