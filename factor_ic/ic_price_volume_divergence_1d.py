...
from data_fetchers.factor_calculator import calculate_price_volume_divergence
...
try:
    SPEC = register_factor(
        FactorSpec(
            factor_name="price_volume_divergence",
            factor_col="price_volume_divergence",
            calculation=calculate_price_volume_divergence,
        )
    )
except SpecRegistrationError as e:
    logger.critical(
        "FactorSpec 注册失败 (factor=price_volume_divergence): %s (%s) (truncated to <=%d chars)",
        str(e)[:_MAX_ERR_LEN],
        type(e).__name__,
        _MAX_ERR_LEN,
    )
...
def _build_parser():
    ...
    parser = argparse.ArgumentParser(description="价跌量缩背离因子 IC 计算器")
...
def main(args=None):
    ...
    try:
        log_factor_summary(result, "价跌量缩背离因子", logger)
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.error(
            "价跌量缩背离因子 IC计算失败 (factor=%s, min_stocks=%s, force_full=%s): %s",
            SPEC.factor_name,
