...
def _render_report(date: str):
    ...
    try:
        ...
        data_results = check_data_freshness(selection_date or date, logger=logger, report_date=date) or []
    except Exception as e:
        logger.warning("check_data_freshness 失败: %s", e)
    try:
        derived_results = check_derived_data_freshness(selection_date or date, logger=logger, report_date=date) or []
    except Exception as e:
