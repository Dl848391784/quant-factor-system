...
# ========== 版本路径设置 ==========
ROOT_DIR = Path(__file__).parent.parent.parent.parent  # 指向 factor_ic_analyzer/
...
BASE_DIR = Path(__file__).parent.parent  # 指向 versions/v3/
OPTIMIZER_DIR = Path(__file__).parent     # 指向 versions/v3/optimizer/
CONFIG_DIR = BASE_DIR / 'config'           # 指向 versions/v3/config/
...
def compute_ic_directions_from_icir(ic_data: Dict, factors: List[str] = None) -> Dict[str, str]:
    """根据 ICIR 动态计算因子方向
...
class WeightOptimizer:
    ...
    def __init__(self, return_col: str = 'forward_return_1d'):
    ...
    def _load_all_ic_series(self, return_col: str = 'forward_return_1d') -> Dict:
        ...
        v3.12 多周期修复（云舟实施）：
        - T+1: 从预生成的 JSON 文件加载（效率高）
        - T+3/T+5: 动态计算 IC（基于因子数据和收益率数据）
        ...
        """
        # T+1: 使用预生成的 IC 文件
        if return_col == 'forward_return_1d':
            return self._load_ic_from_files()
        
        # T+3/T+5: 动态计算 IC
        logger.info(f"[多周期IC] 动态计算 IC（周期={return_col})")
    ...
    def _calculate_ic_dynamically(self, return_col: str) -> Dict:
        ...
        Args:
            return_col: 收益字段名（如 forward_return_3d, forward_return_5d）
        ...
        if not factor_filepath.exists() or not return_filepath.exists():
            logger.error(f"[多周期IC] 缺少因子数据文件")
            return result
        ...
        try:
            ...
            logger.info(f"[多周期IC] 因子数据加载完成: {len(factor_df)} 条")
            
            # 加载收益率数据
            with gzip.open(return_filepath, 'rt', encoding='utf-8') as f:
                return_data = json.load(f).get('data', [])
            return_df = pd.DataFrame(return_data)
            logger.info(f"[多周期IC] 收益率数据加载完成: {len(return_df)} 条")
            
            # 合并数据
            merged_df = factor_df.merge(return_df, on=['date', 'asset'], how='inner')
            logger.info(f"[多周期IC] 数据合并完成: {len(merged_df)} 条")
            
            # 检查收益字段是否存在
            if return_col not in merged_df.columns:
                logger.error(f"[多周期IC] 缺少收益字段: {return_col}")
                return result
            
            # 因子字段映射（因子 ID -> 数据列名）
            factor_columns = {
                'rsi': 'rsi_6',
                'bollinger_pb': None,  # 需要从布林带数据加载
                'volume_ratio': 'volume_ratio_5',
                'turnover_surge': None,  # 需要从换手率数据计算
                'return_3d': None  # 需要从收益数据计算
            }
            
            # 计算各因子 IC
            for factor_id, col_name in factor_columns.items():
                if col_name is None:
                    # 特殊因子需要额外处理（暂用 T+1 IC 数据作为 fallback）
                    logger.warning(f"[多周期IC] 因子 {factor_id} 需要额外数据，使用 T+1 IC fallback")
                    fallback_filepath = ROOT_DIR / FACTOR_RESULT_FILES.get(factor_id, '')
                ...
                if col_name not in merged_df.columns:
                    logger.warning(f"[多周期IC] 因子 {factor_id} 缺少数据列: {col_name}")
                    continue
                ...
                for date in dates:
                    ...
                    factor_values = day_data[col_name].dropna()
                    return_values = day_data[return_col].dropna()
                    ...
                    f = factor_values.loc[common_assets]
                    r = return_values.loc[common_assets]
                ...
                if len(ic_values) > 0:
                    ...
                    logger.info(f"[多周期IC] {factor_id} IC计算完成: {len(valid_dates)} 天, ICIR={icir:.4f}")
                
        except Exception as e:
            logger.error(f"[多周期IC] 动态计算失败: {e}")
            import traceback
            traceback.print_exc()
    ...
    def calculate_combined_icir(
        ...
        公式：组合ICIR ≈ Σ(权重×IC) / sqrt(Σ(权重²×IC²))
        ...
        # P2-修复: 动态计算方向，解决 T+3/T+5 ICIR 为负但方向约束为 positive 的问题
        if ic_directions is None:
            ic_directions = compute_ic_directions_from_icir(self.ic_data, factors)
    ...
    def _generate_weight_combinations_with_direction(
        ...
        for combo in self._generate_weight_combinations(
            ...
            for f in factors:
                ...
                # Phase 2: 放宽方向约束，允许 tolerance 范围内的反向
                if direction == 'positive':
                    # 正向因子：允许 weight >= -tolerance
                    if weight < -tolerance - 1e-10:  # 允许浮点误差
                        valid = False
                        break
                elif direction == 'negative':
                    # 反向因子：允许 weight <= +tolerance
                    if weight > tolerance + 1e-10:  # 允许浮点误差
                        valid = False
                        break
    ...
    def grid_search_collect_top(
        ...
        # P2-修复: 动态计算 IC 方向配置，解决 T+3/T+5 ICIR 为负但方向约束为 positive 的问题
        if ic_directions is None:
            ic_directions = compute_ic_directions_from_icir(self.ic_data, factors)
    ...
    def three_phase_search(
        ...
        for idx, combo in enumerate(candidates_phase1):
            result = self.calculate_combined_icir_with_decay(combo, factors, holding_days=holding_days)
    ...
    def _generate_nearby_combinations(
        ...
        offset_range = 0.3 if step <= 0.05 else 0.2
        offset_values = np.arange(-offset_range, offset_range + step, step)
    ...
    def grid_search_with_backtest(
        ...
        # 导入快速回测验证模块
        try:
            # v3.10 修复：使用相对导入，quick_backtest 在同一目录
            import importlib
            quick_backtest = importlib.import_module('.quick_backtest', package='v3.optimizer')
            QuickBacktestValidator = quick_backtest.QuickBacktestValidator
            parallel_backtest_batch = quick_backtest.parallel_backtest_batch
...
def get_optimizer(return_col: str = 'forward_return_1d') -> WeightOptimizer:
    """获取优化器实例（支持多周期）
    ...
    if return_col not in _optimizer_cache:
        _optimizer_cache[return_col] = WeightOptimizer(return_col=return_col)
        logger.info(f"[优化器缓存] 创建新实例（周期={return_col})")
    
    return _optimizer_cache[return_col]
