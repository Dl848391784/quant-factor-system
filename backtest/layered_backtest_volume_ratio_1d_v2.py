...
使用公共入口 run_layered_backtest，代码量从 ~370 行降至 ~60 行。

功能:
1. 定义因子特有 Config（继承 LayerConfigBase）
2. 调用公共入口 run_layered_backtest
...
main = create_cli_entrypoint(
    ...
    # volume_ratio_5 已在缓存中，无需 factor_calculator
    # 无额外数据源，无需 additional_data_files
)
