...
class KDJJLayerConfig(LayerConfigBase):
    ...
    ic_source: ClassVar[str] = 'factor_ic/result/ic_kdj_j_1d_analysis_result.json'
...
def add_kdj_args(parser: argparse.ArgumentParser) -> None:
    """添加自定义 CLI 参数"""
    parser.add_argument('--kdj-n', type=int, default=DEFAULT_N,
                        help=f'KDJ N 参数，默认 {DEFAULT_N}')


def setup_kdj_calculator(args: argparse.Namespace, calc) -> Callable:
    """包装 factor_calculator"""
    return partial(calc, n=args.kdj_n)
...
if __name__ == '__main__':
    factor_cli_main(
        ...
        add_cli_args=add_kdj_args,
        setup_calculator=setup_kdj_calculator
    )