...
使用公共入口 run_layered_backtest，代码量从 ~510 行降至 ~100 行。

功能:
1. 定义因子特有 Config（继承 LayerConfigBase）
2. 定义因子计算函数 calculate_turnover_surge
3. 调用公共入口 run_layered_backtest
...
def calculate_turnover_surge(
    ...
    参数:
        factor_df: 包含 turnover_rate 列的 DataFrame
        surge_window: 换手率均值窗口
...
def main():
    ...
    try:
        # 创建配置对象（注入 surge_window）
        config = TurnoverSurgeLayerConfig()
        
        # 定义因子计算闭包（注入 surge_window）
        def factor_calc(df):
        ...
        result = run_layered_backtest(
            ...
            additional_data_files={
                'turnover_rate': str(DEFAULT_CACHE_DIR / 'turnover_rate_data.json.gz')
            },
