...
来源：`factor_ic/result/ic_amplitude_1d_analysis_result.json`
...
| 数据源 | 文件 | 必需字段 |
|--------|------|---------|
| 统一数据源 | factor_ic_data.json.gz | date, asset, high, low, close, forward_return_1d |

**因子计算**: 使用 `data_fetchers.factor_calculator.calculate_amplitude`
...
```
backtest/layered_backtest_amplitude_1d.py
```

### 4.2 核心特点

- **薄声明**: 仅定义 factor_name + layer_names ClassVar
- **需计算因子**: 传入 factor_calculator=calculate_amplitude
...
| 文件 | 路径 |
|------|------|
| 回测结果 | `backtest/result/amplitude_layered_backtest.json` |
| 每日明细 | `backtest/result/amplitude_layered_backtest_daily.json.gz` |
