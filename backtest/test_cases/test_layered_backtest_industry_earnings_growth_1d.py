#!/usr/bin/env python3
"""
test_layered_backtest_industry_earnings_growth_1d 测试用例

测试脚本: backtest/layered_backtest_industry_earnings_growth_1d.py
因子计算: data_fetchers/factor_calculator.py::calculate_industry_earnings_growth
...
from backtest.layered_backtest_industry_earnings_growth_1d import IndustryEarningsGrowthLayerConfig
from data_fetchers.factor_calculator import calculate_industry_earnings_growth


class TestIndustryEarningsGrowthLayerConfig:
    """配置类属性验证"""

    def test_factor_name_classvar(self):
        """TC001-01: factor_name 类属性"""
        assert IndustryEarningsGrowthLayerConfig.factor_name == "industry_earnings_growth"

    def test_layer_names_classvar(self):
        """TC001-02: layer_names 类属性为纯标签"""
        assert len(IndustryEarningsGrowthLayerConfig.layer_names) == 5
        assert IndustryEarningsGrowthLayerConfig.layer_names[0] == "lowest"

    def test_layer_descriptions_classvar(self):
        """TC001-03: layer_descriptions 含中文描述"""
        assert len(IndustryEarningsGrowthLayerConfig.layer_descriptions) == 5

    def test_ic_source_default(self):
        """TC001-04: ic_source 默认路径"""
        config = IndustryEarningsGrowthLayerConfig()
        assert config.ic_source_resolved == "factor_ic/result/ic_industry_earnings_growth_1d_analysis_result.json"

    def test_n_layers_derived(self):
        """TC001-05: n_layers 由 len(layer_names) 派生"""
        config = IndustryEarningsGrowthLayerConfig()
        assert config.n_layers == len(IndustryEarningsGrowthLayerConfig.layer_names)
    ...
    def test_layer_names_no_fixed_threshold(self):
        ...
        for name in IndustryEarningsGrowthLayerConfig.layer_names:
...
class TestLayeredBacktestResult:
    ...
    def test_result_file_exists(self):
        """TC003-01: 结果文件存在"""
        result_path = Path("backtest/result/industry_earnings_growth_layered_backtest.json")
    ...
    def test_meta_fields(self):
        ...
        with open(result_path) as fh:
            result = json.load(fh)
        meta = result["meta"]
        assert meta["factor_name"] == "industry_earnings_growth"
