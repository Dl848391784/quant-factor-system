#!/usr/bin/env python3
"""
test_layered_backtest_tail_price_volume_intensity_1d 测试用例

测试脚本: backtest/layered_backtest_tail_price_volume_intensity_1d.py
因子计算: factor_ic/ic_tail_price_volume_intensity.py::calculate_tail_price_volume_intensity
流程文档: backtest/docs/layered_backtest_tail_price_volume_intensity_1d_flow.md
"""
...
from backtest.layered_backtest_tail_price_volume_intensity_1d import (
    TailPriceVolumeIntensityLayerConfig,
)


class TestTailPriceVolumeIntensityLayerConfig:
    """配置类属性验证"""

    def test_factor_name_classvar(self):
        """TC001-01: factor_name 类属性"""
        assert TailPriceVolumeIntensityLayerConfig.factor_name == "tail_price_volume_intensity"

    def test_layer_names_classvar(self):
        """TC001-02: layer_names 类属性为纯标签"""
        assert len(TailPriceVolumeIntensityLayerConfig.layer_names) == 5
        assert TailPriceVolumeIntensityLayerConfig.layer_names[0] == "lowest"

    def test_layer_descriptions_classvar(self):
        """TC001-03: layer_descriptions 含中文描述"""
        assert len(TailPriceVolumeIntensityLayerConfig.layer_descriptions) == 5
        assert TailPriceVolumeIntensityLayerConfig.layer_descriptions[0] == "极低层(量价强度最小)"

    def test_ic_source_default(self):
        """TC001-04: ic_source 默认路径"""
        config = TailPriceVolumeIntensityLayerConfig()
        # 未显式声明时，基类按 factor_name 拼接默认路径
        assert config.ic_source_resolved == ("factor_ic/result/ic_tail_price_volume_intensity_1d_analysis_result.json")

    def test_ic_meta_direction_negative(self):
        """TC001-05: factor_direction = negative（从 IC 文件派生）"""
        config = TailPriceVolumeIntensityLayerConfig()
        # ic_mean = -0.0918 < 0 时 direction = negative
        assert config.factor_direction == "negative"

    def test_n_layers_derived(self):
        """TC001-06: n_layers 由 len(layer_names) 派生"""
        config = TailPriceVolumeIntensityLayerConfig()
        assert config.n_layers == len(TailPriceVolumeIntensityLayerConfig.layer_names)
    ...
    def test_layer_names_dict_generated(self):
        ...
        assert config.layer_names_dict["1"] == "极低层(量价强度最小)"

    def test_layer_names_semantic(self):
        """TC001-08: layer_descriptions 语义描述"""
        # layer_descriptions 应包含"量价强度"相关描述
        for desc in TailPriceVolumeIntensityLayerConfig.layer_descriptions:
            assert "量价强度" in desc

    def test_layer_names_no_fixed_threshold(self):
        """TC001-09: layer_names 纯标签无固定阈值"""
        for name in TailPriceVolumeIntensityLayerConfig.layer_names:
            # 纯标签不含数字阈值
...
class TestLayeredBacktestResult:
    ...
    def test_result_file_exists(self):
        """TC002-01: 结果文件存在"""
        result_path = Path("backtest/result/tail_price_volume_intensity_layered_backtest.json")
    ...
    def test_meta_fields(self):
        ...
        with open(result_path) as f:
            result = json.load(f)
        meta = result["meta"]
        assert meta["factor_name"] == "tail_price_volume_intensity"
