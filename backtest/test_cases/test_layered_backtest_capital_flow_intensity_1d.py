#!/usr/bin/env python3
"""
test_layered_backtest_capital_flow_intensity_1d 测试用例

测试脚本: backtest/layered_backtest_capital_flow_intensity_1d.py
因子计算: data_fetchers/factor_calculator.py::calculate_capital_flow_intensity
...
from backtest.layered_backtest_capital_flow_intensity_1d import (
    CapitalFlowIntensityLayerConfig,
)
from data_fetchers.factor_calculator import calculate_capital_flow_intensity


class TestCapitalFlowIntensityLayerConfig:
    """配置类属性验证"""

    def test_factor_name_classvar(self):
        """TC001-01: factor_name 类属性"""
        assert CapitalFlowIntensityLayerConfig.factor_name == "capital_flow_intensity"

    def test_layer_names_classvar(self):
        """TC001-02: layer_names 类属性为纯标签"""
        assert len(CapitalFlowIntensityLayerConfig.layer_names) == 5
        assert CapitalFlowIntensityLayerConfig.layer_names[0] == "lowest"

    def test_layer_descriptions_classvar(self):
        """TC001-03: layer_descriptions 含中文描述"""
        assert len(CapitalFlowIntensityLayerConfig.layer_descriptions) == 5

    def test_ic_source_default(self):
        """TC001-04: ic_source 默认路径（IC文件不存在时跳过）"""
        try:
    ...
    def test_n_layers_derived(self):
        ...
        try:
            ...
            pytest.skip("IC结果文件不存在")
        assert config.n_layers == len(CapitalFlowIntensityLayerConfig.layer_names)

    def test_layer_names_dict_generated(self):
        """TC001-06: layer_names_dict 使用 layer_descriptions（IC文件不存在时跳过）"""
        try:
            config = CapitalFlowIntensityLayerConfig()
        except FileNotFoundError:
            pytest.skip("IC结果文件不存在")
    ...
    def test_layer_names_no_fixed_threshold(self):
        ...
        for name in CapitalFlowIntensityLayerConfig.layer_names:
            assert not any(c.isdigit() for c in name)

    def test_factor_direction_literal_type(self):
        """TC001-08: factor_direction 类型约束（IC文件不存在时跳过）"""
...
class TestLayeredBacktestResult:
    ...
    def test_result_file_exists(self):
        """TC003-01: 结果文件存在"""
        result_path = Path("backtest/result/capital_flow_intensity_layered_backtest.json")
    ...
    def test_meta_fields(self):
        ...
        with open(result_path) as fh:
            result = json.load(fh)
        meta = result["meta"]
        assert meta["factor_name"] == "capital_flow_intensity"
...
class TestLayeredBacktestExecution:
    ...
    def test_factor_direction_derives_long_short(self):
        """TC004-02: factor_direction 决定多空组合（IC文件不存在时跳过）"""
        try:
            config = CapitalFlowIntensityLayerConfig()
        except FileNotFoundError:
            pytest.skip("IC结果文件不存在")
