...
## A2 增强 (2026-07-19 追加): footer 显示条目明细

**触发**: 用户问"证据链能否保证准确 + 能否显示内容人工顺便检查准确性".

**准确性分层** (诚实边界): 机器保"真实性"(READ/VERIFY 自动记录不可伪造 + VERIFY路径校验 + CONTRACT锚点校验), 不能保"准确性"(推理质量/读对文件/结论逻辑). 准确性靠**人审**——需把证据内容给用户看.

**改动**: `on_transform_llm_output` footer 从"一行计数"升级为"计数 + 条目明细块" (`_format_evidence_detail`). 防膨胀: 每条≤110字符, 最多10条, 超限折叠"其余N条略, 完整见文件".

**验证**: 真实证据链渲染明细 ✓; 12条→10条+折叠 ✓; 截断省略号 ✓.
