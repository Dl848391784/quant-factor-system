...
## 快速参考
   ...
   103||| 87 | 增量更新检查日期是否最新 | 跳过逻辑必须检查缓存中该股票的最新日期 >= T-1（昨天），而非只检查股票是否存在 |
   104||| 19 | 常量定义在 import 之后 | PEP 8 顺序：docstring → __future__ → 标准库 → 第三方 → 本地 → 常量 |
