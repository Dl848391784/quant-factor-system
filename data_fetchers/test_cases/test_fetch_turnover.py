...
class TestCoreFlow:
    ...
    def test_load_cache_not_exists(self, test_logger, monkeypatch):
        ...
        # Mock CACHE_FILE
        monkeypatch.setattr("data_fetchers.fetch_turnover.CACHE_FILE", Path("/nonexistent/path/file.json.gz"))
    ...
    def test_load_cache_valid_json(self, test_logger, temp_cache_file, monkeypatch):
        ...
        test_data = {
            ...
            "data": [{"date": "2026-05-27", "asset": "600000", "turnover_rate": 2.5}],
        }
        ...
        # Mock CACHE_FILE
        monkeypatch.setattr("data_fetchers.fetch_turnover.CACHE_FILE", temp_cache_file)
    ...
    def test_save_cache_atomic_write(self, test_logger, temp_cache_file, monkeypatch):
        ...
        monkeypatch.setattr("data_fetchers.fetch_turnover.RESULT_DIR", temp_cache_file.parent)
        monkeypatch.setattr("data_fetchers.fetch_turnover.CACHE_FILE", temp_cache_file)
...
class TestDataMerge:
    ...
    def test_merge_new_records(self, test_logger):
        """TC003-1: 合并新数据"""
        existing = None
        new_records = [{"date": "2026-05-27", "asset": "600000", "turnover_rate": 2.5}]
    ...
    def test_merge_with_existing(self, test_logger):
        ...
        existing = {
            ...
            "data": [{"date": "2026-05-26", "asset": "600000", "turnover_rate": 1.5}],
        }
        new_records = [{"date": "2026-05-27", "asset": "600000", "turnover_rate": 2.5}]
    ...
    def test_merge_dedup(self, test_logger):
        ...
        existing = {
            ...
            "data": [{"date": "2026-05-27", "asset": "600000", "turnover_rate": 1.5}],
        }
        new_records = [
