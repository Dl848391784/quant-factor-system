...
def validate_dataframe_columns(
    df: pd.DataFrame, required_cols: list[str], df_name: str | None = None, logger: logging.Logger | None = None
) -> None:
    """
    ...
    if not isinstance(df, pd.DataFrame):
        logger.error("DataFrame 参数类型错误: %s, 实际类型: %s", df_name, type(df).__name__)
        raise TypeError(f"{df_name} 参数必须是 pandas.DataFrame 类型")

    # 边界处理：required_cols 参数校验
    if not required_cols:
        logger.error("required_cols 参数为空列表: %s", df_name)
    ...
    if missing_cols:
        ...
        error_msg = f"{df_name} 缺少必需列: {missing_cols}\n可用列: {available_cols}"

        # 日志记录（便于审计追溯）
        logger.error("DataFrame 列名校验失败: %s, 缺失: %s, 可用: %s", df_name, missing_cols, available_cols)

        raise ValueError(error_msg)

    # 验证通过日志（只记录关键信息，避免冗长输出）
    logger.debug("%s 列名校验通过，共 %s 列", df_name, len(required_cols))
