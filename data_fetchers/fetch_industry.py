...
# 申万2021版行业代码映射（一级代码前两位 -> 一级行业名称）
# 注意：此映射基于 akshare 返回的 industry_code 前两位提取
# - industry_code 格式：一级代码(2位)+二级代码(2位)，如 '2101'
# - 前两位代码对应申万2021一级行业分类
# - 已根据 akshare stock_industry_clf_hist_sw 实际返回数据建立映射
# - 未核对申万官方标准，仅供参考
# TODO: 核对申万2021官方行业分类标准，确认一级代码与行业名称对应关系
# 参考: https://www.swsindex.com/hsi/IndexList.aspx
SW_INDUSTRY_CODE_MAP: dict[str, str] = {
    # 一级行业代码 -> 行业名称（基于实际数据建立，未核对官方标准）
    '11': '农林牧渔',
    '21': '基础化工',
    '22': '基础化工',  # 前两位 '22' → 基础化工（二级归属待核实）
    '23': '钢铁',
    '24': '有色金属',
    '25': '汽车',
    '26': '家用电器',
    '27': '电子',
    '28': '汽车',      # 前两位 '28' → 汽车（二级归属待核实）
    '31': '商贸零售',
    '32': '医药生物',
    '33': '家用电器',  # 前两位 '33' → 家用电器（二级归属待核实）
    '34': '食品饮料',
    '35': '纺织服饰',
    '36': '轻工制造',
    '37': '医药生物',  # 前两位 '37' → 医药生物（二级归属待核实）
    '41': '公用事业',
    '42': '交通运输',
    '43': '房地产',
    '44': '建筑材料',
    '45': '社会服务',
    '46': '综合',
    '47': '综合',      # 前两位 '47' → 综合（二级归属待核实）
    '48': '银行',
    '49': '非银金融',
    '51': '综合',      # 前两位 '51' → 综合（二级归属待核实）
    '61': '建筑材料',  # 前两位 '61' → 建筑材料（二级归属待核实）
    '62': '建筑装饰',
    '63': '电力设备',
    '64': '机械设备',
    '65': '国防军工',
