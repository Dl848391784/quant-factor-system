        ...
        try:
            ...
            if result['actual_date'] == expected_t_minus_1:
                ...
                logger.warning(
                    ...
                    source_name, result['actual_date'], expected_t_minus_1
                )
...
def _get_nested_field(data: Dict, field_path: str) -> Optional[str]:
    """从嵌套字典中获取字段值
...
def _extract_date_from_json_content(content: str, date_field: str) -> Optional[str]:
...
def _generate_data_check_section(data_results: List[Dict], derived_results: List[Dict]) -> List[str]:
    ...
    for item in data_results:
        lines.append(
            f"{item['source']:<20} "
            f"{item['description']:<24} "
            f"{item['actual_date']:>12} "
            f"{item['status_symbol']:>10}"
        )
