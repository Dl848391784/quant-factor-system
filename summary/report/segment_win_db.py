...
# ── 列定义 ────────────────────────────────────────────────────────────
SEGMENT_WIN_COLUMNS = [
    "pipeline", "selection_date", "trade_date", "weight_method",
    "n_segments", "n_total", "segment_label",
    "wins", "total", "win_rate", "created_at",
]

SEGMENT_STOCK_COLUMNS = [
    "pipeline", "selection_date", "segment_label",
    "asset", "composite_value", "rank", "created_at",
]
...
def save_segment_stock_details(
    ...
    if not existing.empty:
        mask = (existing["pipeline"] == pipeline) & (existing["selection_date"] == selection_date)
        existing = existing[~mask]

    combined = pd.concat([existing, new_df], ignore_index=True)
    combined.to_parquet(fp, index=False)
    logger.info("segment_stock_details: %s/%s 写入 %d 只 → %s (累计 %d 行)",
                pipeline, selection_date, len(rows), fp.name, len(combined))
...
def save_segment_win_rates(
    ...
    combined = pd.concat([existing, new_df], ignore_index=True)
    combined.to_parquet(fp, index=False)
    logger.info("segment_win_rates: %s/%s 写入 %d 段 → %s (累计 %d 行)",
                pipeline, selection_date, len(rows), fp.name, len(combined))
