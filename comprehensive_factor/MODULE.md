...
> 本节定义代码注释中引用数据值的规范，避免误导性注释。

### 问题类型

**问题：** 注释中硬编码数据值（如 ICIR ≈ -1.97），但实际数据已变化，注释与实际不符，产生误导。
...
def get_weights(...):
    ...
    处理负ICIR：
    - 反向因子ICIR为负（如 volume_ratio ICIR ≈ -1.97）  # ❌ 数据过时
...
def get_weights(...):
    ...
    实际 ICIR 值（见 factor_ic/result/*.json）：  # ✓ 数据来源说明
...
| 场景 | 要求 | 示例 |
|------|------|------|
| 引用统计值 | 必须说明数据来源 + 时间范围 | `ICIR=0.3058（见 factor_ic/result/*.json，2024-03~2026-05）` |
| 引用阈值 | 必须说明依据（业界标准/统计结果） | `|ic_mean|>0.03（业界公认的最低有效阈值）` |
| 引用公式 | 必须说明语义，而非只写公式 | `ICIR = IC均值/IC标准差，反映预测稳定性` |

**数据来源格式：**
- 文件路径：`见 factor_ic/result/*.json`
- 模块名：`由 weight_engine.py ICIRWeightMethod 动态计算`
- 时间范围：`（2024-03-27~2026-05-14）`

**适用范围：**
- 所有引用实际数据值的注释
- 公共模块的 docstring
- Config 类注释中引用 IC/ICIR 等统计值

---

| 版本 | 日期 | 变更内容 |
|------|------|----------|
| v1.16 | 2026-05-26 | 新增 --auto_select CLI 参数规范：自动因子筛选（基于 ICIR 和高相关性）；修复 FACTOR_NAME_TO_COL_MAP 列名映射（与数据源一致）；修正 create_cli_entrypoint 参数顺序 |
| v1.15 | 2026-05-24 | 新增注释数据来源规范：引用数据值必须说明来源和时间范围 |
